...
# HyprGlass keys are only valid while the plugin is loaded (after a Hyprland
# update it stays unloaded until `hyprpm update`); setting them otherwise is
# a config error. So they go in a guarded, separate call.
glass_lua="if hl.plugin and hl.plugin.hyprglass then hl.config({ plugin = { hyprglass = { enabled = $glass } } }) end"
...
hyprctl eval "$glass_lua" >/dev/null 2>&1 || true
